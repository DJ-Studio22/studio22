...
DEPLOY.md and were carried out in Phase 31, when the site moved to
studio22.games.
...
- FIXED (was a bug in the original prototype, confirmed there): Comet's stardust used to grow without bound — waves added five and collection respawned them. The field is now capped at DUST_ON_FIELD (6) and refilled only on collection; waves add threat, not supply. Late waves are meant to be survived by flying well, not by walking through a carpet of pickups
...
- DONE: the arcade hub surfaces inputRequirement on game cards -- a KEYBOARD REQUIRED badge on Keystroke, the only "keyboard" entry; everything else is "universal" -- so a player on a phone sees that a keyboard-only game will not work before tapping in
